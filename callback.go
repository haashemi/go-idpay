...
type PostCallback struct {
	Status       int       `json:"status"`         // Status is Transaction Status
	TrackID      int       `json:"track_id"`       // TrackIS is IDPay's track id
	...
	Amount       int       `json:"amount"`         // Amount is submitted amount on creating the transaction
